...
class UnetSkipConnectionEBlock4(nn.Module):
    def __init__(self, outer_nc, inner_nc, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d,
                 use_dropout=False):
        super(UnetSkipConnectionEBlock4, self).__init__()
        downconv = nn.Conv2d(outer_nc, inner_nc, kernel_size=4,
                             stride=2, padding=0)
...
class Encoder2(nn.Module):
    def __init__(self, input_nc, ngf=64, res_num=4, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        Encoder_2 = UnetSkipConnectionEBlock(ngf, ngf * 2, norm_layer=norm_layer, use_dropout=use_dropout)
        Encoder_3 = UnetSkipConnectionEBlock(ngf * 2, ngf * 4, norm_layer=norm_layer, use_dropout=use_dropout)
        Encoder_4 = UnetSkipConnectionEBlock4(ngf * 4, ngf * 8, norm_layer=norm_layer, use_dropout=use_dropout)
...
class Sideoutput1(nn.Module):
    def __init__(self,in_channel,out_channel):
        super(Sideoutput1, self).__init__()
        self.up_conv = nn.Conv2d(in_channel,out_channel,kernel_size=1,stride=1,padding=0)
        self.up_transpose = nn.ConvTranspose2d(3,3,kernel_size=6,stride=4,padding=1)
    def forward(self,out_feature1):
        fea1 = self.up_conv(out_feature1)
        fake1 = self.up_transpose(fea1)
        return fake1
...
class Sideoutput2(nn.Module):
    def __init__(self,in_channel,out_channel):
        ...
        self.up_conv = nn.Conv2d(in_channel,out_channel,kernel_size=1,stride=1,padding=0)
        self.up_transpose = nn.ConvTranspose2d(3,3,kernel_size=4,stride=2,padding=1)
    def forward(self,out_feature1):
        fea1 = self.up_conv(out_feature1)
        fake1 = self.up_transpose(fea1)
        return fake1

...
from Repair.util2.Selfpatch import Selfpatch
...
class BASE(nn.Module):
    def __init__(self, inner_nc,batch_size):
        super(BASE, self).__init__()
        se = SELayer(inner_nc, 16)
        model = [se]
        self.batch_size = batch_size
        gus = util.gussin(1.5,batch_size=self.batch_size).cuda()   #要改batch_size必须改动这个
...
class PCconv(nn.Module):
    def __init__(self,batch_size):
        ...
        self.base= BASE(512,batch_size=batch_size)
        self.batch_size = batch_size
        seuqence_3 = []
        seuqence_5 = []
        seuqence_7 = []
    ...
    def forward(self, input, mask):
        #显示mask
        out_mask = mask.detach().cpu().numpy().squeeze()
        ...
        # The first three layers are Texture/detail 前三个feature拼接学习纹理细节
        # The last three layers are Structure       后三个feature拼接学习结构细节
